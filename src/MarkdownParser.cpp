...
var::JsonObject MarkdownParser::extract_code_snippets(
  ...
  if (markdown_input_file.open(file_path, OpenFlags::read_only()) < 0) {
    printer().error(
  ...
  while ((line = markdown_input_file.gets()).is_empty() == false) {

    if (is_on_code_block && (line.find("```") == String::npos)) {
    ...
    if (line.find("```") == 0) {
      if (is_on_code_block) {
        ...
        Tokenizer header(code_block_header, Tokenizer::Delimeters(":\n"));
        ...
        if (
          ...
          code_block_object.insert("section", JsonString(header.at(1)));

          code_block_object.insert("line", JsonInteger(start_code_line));

          code_block_object.insert(
            "code",
            JsonString(Base64::encode(code_block_string)));
  ...
  if (code_block_array.count() > 0) {
    ...
    name.replace(String::ToErase(".md"), String::ToInsert(""));

    result.insert("name", JsonString(name));

    result.insert(
      "destinationDirectory",
      JsonString(properties.at("output").to_string() << "/code/" << name));

    result.insert("source", JsonString(file_path));

    result.insert("template", properties.at("template"));

    result.insert("buildDirectory", properties.at("buildDirectory"));

    result.insert("cmakeOptions", properties.at("cmakeOptions"));

    result.insert("makeOptions", properties.at("makeOptions"));

    result.insert("snippets", code_block_array);
  }
