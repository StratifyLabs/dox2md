...
class DoxygenMember : public DoxygenObject {
  DoxygenMember(JsonObject object)
    : DoxygenObject(object), m_location(object.at("location").to_object()) {
...
public:
  ...
  static bool is_file_name_match_for_pages(
    ...
    if (pages.find(lookup_file_category(file_name)) < pages.count()) {
      return true;
    }
  ...
  static String lookup_file_category(const String &file_name) {
    if (
      (file_name.find("8hpp.xml") != String::npos)
      || (file_name.find("8h.xml") != String::npos)) {
      return "header";
    }

    if (
      (file_name.find("8c.xml") != String::npos)
