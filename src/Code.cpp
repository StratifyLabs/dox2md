...
int Code::generate(const var::String &snippets_path) {
  ...
  JsonObject project_object
    = JsonDocument().load(fs::File::Path(snippets_path)).to_object();

  JsonArray snippets_array = project_object.at("snippets").to_array();
  ...
  String build_directory = project_object.at("buildDirectory").to_string();

  printer().debug("build code in directory %s", build_directory.cstring());

  create_code_project(
    fs::File::SourcePath(project_object.at("template").to_string()),
    fs::File::DestinationPath(destination_directory));

  insert_code_snippets(snippets_array, destination_directory);
  ...
  if (is_build == "true") {
    ...
    printf("\n--------------------Start Build Output-------------------\n");
    build_code(String() << destination_directory << "/" << build_directory);
    printf("\n--------------------End Build Output-------------------\n");
  }
...
int Code::insert_code_snippets(
  ...
  main_file_path = String() << destination_path << "/src/main.cpp";

  if (main_file.open(main_file_path, OpenFlags::read_only()) < 0) {
  ...
  while (((line = main_file.gets()).is_empty() == false)
  ...
  for (u32 i = 0; i < snippets_array.count(); i++) {
    ...
    String section = snippet_object.at("section").to_string();

    if (section == "include") {
      String encoded_string = snippet_object.at("code").to_string();
  ...
  for (u32 i = 0; i < snippets_array.count(); i++) {
    ...
    if (section == "main") {
      ...
      printer().debug("insert block in main - '%s'", encoded_string.cstring());
  ...
  if (
    main_file.create(
      String() << destination_path << "/src/main.cpp",
